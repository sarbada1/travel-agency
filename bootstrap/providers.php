<?php


return [
    App\Providers\AccountType\AccountTypeServiceProvider::class,
    App\Providers\Address\AddressServiceProvider::class,
    App\Providers\AppServiceProvider::class,
    App\Providers\Blog\BlogServiceProvider::class,
    App\Providers\AttributeGroup\AttributeGroupServiceProvider::class,
    App\Providers\Attribute\AttributeServiceProvider::class,
    App\Providers\Category\CategoryServiceProvider::class,
    App\Providers\GlobalCreate\PermissionGlobalCreateServiceProvider::class,
    App\Providers\GlobalCreate\PermissionGlobalCreateServiceProvider::class,
    App\Providers\GlobalDataShare\GlobalDataShareServiceProvider::class,
    App\Providers\GlobalDataShare\GlobalDataShareServiceProvider::class,
    App\Providers\News\NewsServiceProvider::class,
    App\Providers\Page\PageServiceProvider::class,
    App\Providers\Permission\PermissionServiceProvider::class,
    App\Providers\Role\RoleServiceProvider::class,
    App\Providers\Setting\SettingServiceProvider::class,
    App\Providers\User\UserServiceProvider::class,
    App\Providers\Ad\AdServiceProvider::class,
    App\Providers\AttributeGroup\AttributeGroupServiceProvider::class,
    App\Providers\PackageAttribute\PackageAttributeServiceProvider::class,
    App\Providers\AttributeValue\AttributeValueServiceProvider::class,
    App\Providers\TourPackage\TourPackageServiceProvider::class,
    App\Providers\ViewServiceProvider::class,
    
];
